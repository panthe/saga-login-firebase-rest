...
const dispatchWithResolve = (action: Action, dispatch: Dispatch) =>
...
export default function requestMiddleware(): MiddlewareFunction {
  return ({ dispatch, getState }) => next => action => {
    ...
    if (
      Object.values(ERequestMiddlewareActionTypes).includes(action.type) &&
          ...
          apiRefreshToken({
            ...
            .then(response => {
              dispatchWithResolve(
                ...
                Cookies.set('expiresIn', response.expires_in);
                return next(action);
              });
            })
            .catch(error => {
